        ...
        for inner_iter, data in enumerate(loader_fb['train']):
            ...
            for j in range(repeat_permute):
                ...
                for index in range(overlap_tag_for_tea.shape[1]):
                    ...
                    # overlap_tag_for_tea[j, index] = tcjsd
                    
                    overlap_tag_for_tea[j, index] = dist_loss(h1_unpermu, h2)
                    # omega = pd_diff(h2_unpermu,labels,h1_unpermu)
                    # omega = 1 - omega
                    # overlap_tag_for_tea[j, index] = dist_dkd(h1_unpermu, h2, labels, 7,1,0.7,omega)  # target, alpha, beta, temperature
