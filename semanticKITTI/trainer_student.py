...
from pc_processor.utils.functions import pd_diff

class Trainer(object):
    def __init__(self, settings: Option, model_T: nn.Module, model_S: nn.Module,recorder=None):
        # init params
        self.settings = settings
        self.recorder = recorder
        self.model_T = model_T.cuda()
        self.model_S = model_S.cuda()
        self.remain_time = pc_processor.utils.RemainTime(
            self.settings.n_epochs)
        
        # load teacher model parametr
        configs_path = '/home/elon/Projects/MFH-main/semanticKITTI/experiments/PMF-SemanticKitti/log_SemanticKitti_PMFNet-resnet34_bs16-lr0.001_baseline_timestamp/checkpoint/best_IOU_model.pth'
        model_T_state_dict = torch.load(configs_path)
        self.model_T.load_state_dict(model_T_state_dict)
        self.model_T.eval()
        ...
        if self.settings.n_gpus > 1:
            if self.settings.distributed:
                # sync bn
                self.model_T = pc_processor.layers.sync_bn.replaceBN(
                    self.model_T).cuda()
                self.model_T = nn.parallel.DistributedDataParallel(
                    self.model_T, device_ids=[self.settings.gpu]) #  find_unused_parameters=True

                self.model_S = pc_processor.layers.sync_bn.replaceBN(
                    self.model_S).cuda()
                self.model_S = nn.parallel.DistributedDataParallel(
                    self.model_S, device_ids=[self.settings.gpu]) #  find_unused_parameters=True
            else:
                self.model_T = nn.DataParallel(self.model_T)
                self.model_S = nn.DataParallel(self.model_S)
    ...
    def _initOptimizer(self):
        # check params
        adam_params = [{"params": self.model_S.lidar_stream.parameters()}]

        adam_opt = torch.optim.AdamW(
            params=adam_params, lr=self.settings.lr)

        # sgd_params = [
        #     {"params": self.model_T.camera_stream_encoder.parameters()},
        #     {"params": self.model_T.camera_stream_decoder.parameters()}]
    ...
    def _initDataloader(self):
        if self.settings.dataset == "SemanticKitti":
            ...
            trainset = pc_processor.dataset.semantic_kitti.SemanticKitti(
                ...
                sequences= [0,1,2,3,4,5,6,7,9,10],#[0,1,2,3,4,5,6,7,9,10],
    ...
    def _initCriterion(self):
        ...
        criterion['dkd_loss'] = pc_processor.loss.DKDLoss(temp=0.7,alpha=7.0,beta=1.0)
    ...
    def run(self, epoch, mode="Train"):
        ...
        for i, (input_feature, input_mask, input_label) in enumerate(dataloader):
            ...
            if mode == "Train":
                with torch.no_grad():
                    camera_pred, camera_logits = self.model_T(img_feature)
                    
                lidar_pred,lidar_logits = self.model_S(pcd_feature)
                
                omega, _ = pd_diff(camera_pred,input_label,lidar_pred)
                ...
                xm_loss = (self.criterion["kl_loss"](lidar_pred_log, camera_pred)*omega.unsqueeze(1)).mean()
                # xm_loss = (self.criterion["kl_loss"](lidar_pred_log, camera_pred)).mean()
                # xm_loss = self.criterion['dkd_loss'](lidar_logits, camera_logits, input_label,mask_t)
                ...
                total_loss = loss_foc + loss_lov * self.settings.lambda_  + 0*xm_loss
                ...
                with torch.no_grad():
                    lidar_pred,_ = self.model_S(pcd_feature)
                    camera_pred,_ = self.model_T(img_feature)
                    ...
                    #     loss_foc_cam + loss_lov_cam * self.settings.lambda_ + \
                    #     loss_per * self.settings.gamma
                    # mask,mask_t = pd_diff(camera_logits,input_label,lidar_logits)
                    # xm_loss = self.criterion['dkd_loss'](lidar_logits, camera_logits, input_label,mask_t)
                    # xm_loss = (self.criterion["kl_loss"](lidar_pred_log, camera_pred)*(1-mask_t).unsqueeze(1)).mean()
                    xm_loss = (self.criterion["kl_loss"](lidar_pred_log, camera_pred)).mean()   
                    total_loss = loss_foc + loss_lov * self.settings.lambda_ + 0*xm_loss
            ...
            if self.recorder is not None:
                ...
                log_str += "LR {:0.5f} Loss {:0.4f}".format(lr, loss.item())
                log_str += "Acc {:0.4f} IOU {:0.4F} Recall {:0.4f} Entropy {:0.4f} ".format(
                    mean_acc.item(), mean_iou.item(), mean_recall.item(), entropy_meter.avg)
