...
class Experiment(object):
    def __init__(self, settings: Option):
        ...
        self.epoch_start = 0
        # init model
        self.model_T = pc_processor.models.CameraNet(
            img_channels=3,
        ...
        self.model_S = pc_processor.models.LidarNet(
            pcd_channels=5,
            nclasses=self.settings.nclasses,
            base_channels=32,
        )        
        ...
        # init trainer
        self.trainer = trainer_student.Trainer(
            self.settings, self.model_T, self.model_S, self.recorder)
        # load checkpoint
    ...
    def _loadCheckpoint(self):
        ...
        if self.settings.pretrained_model is not None:
            ...
            new_state_dict = self.model_S.state_dict()
            for k, v in state_dict.items():
                if k in new_state_dict.keys():
                    ...
                    print("diff key: ", k)
            self.model_S.load_state_dict(new_state_dict)
        ...
        if self.settings.checkpoint is not None:
            ...
            self.model_S.load_state_dict(checkpoint_data["model"])
                ...
                if self.recorder is not None:
                    ...
                    for k, v in val_result.items():
                        if v >= best_val_result[k]:
                            ...
                            best_val_result[k] = v
                            torch.save(self.model_S.state_dict(), saved_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument("config_path", type=str, default='./config_server_kitti.yaml',metavar="config_path",
                         help="path of config file, type: string")
    parser.add_argument("--id", type=int, metavar="experiment_id", required=False,
                        help="id of experiment", default=1)
