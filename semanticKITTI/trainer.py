...
class Trainer(object):
    def __init__(self, settings: Option, model: nn.Module, recorder=None):
        ...
        [self.aux_optimizer] = self._initOptimizer()
    ...
    def run(self, epoch, mode="Train"):
        ...
        for i, (input_feature, input_mask, input_label) in enumerate(dataloader):
            ...
            if mode == "Train":
                camera_pred = self.model(img_feature)
        ...
        if self.recorder is not None:
            ...
            log_str = ">>> {} Loss {:0.4f} Acc {:0.4f} IOU {:0.4F} Recall {:0.4f}".format(
                mode, loss_meter.avg, mean_acc_img.item(), mean_iou_img.item(), mean_recall_img.item())
            self.recorder.logger.info(log_str)

        result_metrics = {
            "Acc": mean_acc_img.item(),
            "IOU": mean_iou_img.item(),
            "Recall": mean_recall_img.item(),
            "last": 0
        }
