...
from pc_processor.dataset.semantic_kitti.parser import SemanticKitti
import numpy as np
import os
import shutil
...
def createFovDataset(src_root, dst_root, seq):
    dataset = SemanticKitti(
        root=src_root,
        sequences=[seq],
        config_path="/home/elon/Projects/PMF-master/pc_processor/dataset/semantic_kitti/semantic-kitti.yaml"
    )
...
if __name__ == "__main__":
    for seq in range(0, 11):
        createFovDataset(
            src_root="/data/elon/semantic_kitti/sequences", # path to the original semantic-kitti dataset
            dst_root="/data/elon/semantic-kitti-fov_1/sequences", # path to the generated semantic-kitti-fov dataset
            seq=seq
        )